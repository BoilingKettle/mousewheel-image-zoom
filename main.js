...
class MouseWheelZoomPlugin extends obsidian.Plugin {
    ...
    registerEvents(currentWindow) {
        ...
        this.registerDomEvent(doc, "wheel", (evt) => {
            if (this.isKeyHeldDown) {
                ...
                const targetIsPdfEmbed = eventTarget.closest('.pdf-cropped-embed') !== null;

                if (targetIsCanvas || targetIsCanvasNode || targetIsImage || targetIsPdfEmbed) {
                    this.disableScroll(currentWindow);
                }
                ...
                else if (targetIsPdfEmbed || targetIsImage) {
    ...
    */
    ...
    /**
     * Handles zooming with the mousewheel on an image or PDF embed
     * @param evt wheel event
     * @param eventTarget targeted element
     * @private
     */
    handleZoom(evt, eventTarget) {
        return __awaiter(this, void 0, void 0, function* () {
            const pdfEmbedDiv = eventTarget.closest('.pdf-cropped-embed');
            if (pdfEmbedDiv) {
                // 获取当前宽度，如果没有设置则使用默认宽度
                const currentWidth = parseInt(pdfEmbedDiv.getAttribute('width')) || this.settings.initialSize;

                let newWidth = currentWidth;
                if (evt.deltaY < 0) {
                    newWidth += this.settings.stepSize;
                } else if (evt.deltaY > 0 && newWidth > this.settings.stepSize) {
                    newWidth -= this.settings.stepSize;
                }

                // 更新DOM元素宽度
                pdfEmbedDiv.setAttribute('width', newWidth.toString());
                const imgElement = pdfEmbedDiv.querySelector('img');
                if (imgElement) {
                    imgElement.setAttribute('width', newWidth.toString());
                }

                // 获取并更新文件内容
                const activeFile = yield this.getActivePaneWithImage(pdfEmbedDiv);
                let fileText = yield this.app.vault.read(activeFile);
                const originalFileText = fileText;

                // 构建正则表达式匹配PDF引用
                const pdfSrc = pdfEmbedDiv.getAttribute('src');
                const escapedSrc = escapeRegex(pdfSrc);

                // 检查是否已有宽度设置
                const hasWidthRegex = new RegExp(`\\[\\[${escapedSrc}\\|([^\\|]+?)\\|(\\d+)\\]\\]`);
                const noWidthRegex = new RegExp(`\\[\\[${escapedSrc}\\|([^\\|]+?)\\]\\]`);

                const hasWidthMatch = fileText.match(hasWidthRegex);
                const noWidthMatch = fileText.match(noWidthRegex);

                if (hasWidthMatch) {
                    // 更新已有宽度
                    fileText = fileText.replace(hasWidthRegex, `[[${pdfSrc}|$1|${newWidth}]]`);
                } else if (noWidthMatch) {
                    // 添加宽度设置
                    fileText = fileText.replace(noWidthRegex, `[[${pdfSrc}|$1|${newWidth}]]`);
                }
